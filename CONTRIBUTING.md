...
## Working with Types
This codebase is implemented using a well-typed approach where we rely heavily on the Typescript compiler for catching & enforcing certain kinds of checks we would like to perform.
...
For example, an onchain address and a lightning network payment request are both strings, but they aren't interchangeable as a data type. Instead of using `string` type for these types we would define as follows using a "unique symbol" trick:
...
These are types mostly used in function signature type definitions. They derive from implemented error classes and need to be imported in a special way to thet type declaration file.
...
In the places where we would like to work with types defined in imported libraries, we have two options. When we would like to use them directly, we can simply import them. If we would like to re-use a type to create our own types, we would have to use our import trick to get those types into our declaration files.
...
For example:
```ts
const myFunction = async (myArg: MyArg): ThingResult => {
...
// in other file
const myFunction = async ({ myFirstArg, mySecondArg }: MyFuncArgs): ThingResult => {
  return doThing(myFirstArg, mySecondArg)
}
```
...
```
## Architecture
We use hexagonal architecture pattern (some link). 
